...
const actions = {
  async login({ commit }, user) {
    ...
    const response = await axios.post(
      "http://localhost:8000/wp-json/jwt-auth/v1/token?username=" + user[0].username + "&password=" + user[0].password);
  ...
  async checkLoginState({ commit }) {
    ...
    try {
      await axios.post(
        "http://localhost:8000/jwt-auth/v1/token/validate",
        {},
        {
          headers: {
...
const store = {
  ...
  state: {
    ...
    userAuthorId: localStorage.getItem("authorId") || "",
  },
